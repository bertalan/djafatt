"""Views for self-invoice (autofattura) CRUD (T14)."""
...
from .forms import InvoiceLineFormSet, PaymentDueFormSet, SelfInvoiceForm
from .models import SelfInvoice


class SelfInvoiceListView(LoginRequiredMixin, GroupPermissionMixin, ListView):
    permission_required = "invoices.view_invoice"
    model = SelfInvoice
    template_name = "self_invoices/index.html"
    ...
    def get_template_names(self):
        if self.request.htmx:
            return ["self_invoices/partials/_table.html"]
        return [self.template_name]


class SelfInvoiceCreateView(LoginRequiredMixin, GroupPermissionMixin, CreateView):
    permission_required = "invoices.add_invoice"
    model = SelfInvoice
    form_class = SelfInvoiceForm
    template_name = "self_invoices/form.html"
    success_url = reverse_lazy("self-invoices-index")

    def dispatch(self, request, *args, **kwargs):
        fiscal_year = request.session.get("fiscal_year", date.today().year)
        if fiscal_year < date.today().year:
            messages.error(request, "Non puoi creare autofatture in un anno fiscale passato.")
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, f"Autofattura {invoice.number} creata.")
        return HttpResponseRedirect(self.success_url)


class SelfInvoiceEditView(LoginRequiredMixin, GroupPermissionMixin, UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not self.object.is_editable():
            ...
            if dues_formset.is_valid():
                ...
                messages.success(request, f"Pagamenti autofattura {self.object.number} aggiornati.")
    ...
    def form_valid(self, form):
        ...
        dues_formset.save()
        invoice.calculate_totals()
        messages.success(self.request, f"Autofattura {invoice.number} aggiornata.")
        return HttpResponseRedirect(self.success_url)


class SelfInvoiceDeleteView(LoginRequiredMixin, GroupPermissionMixin, DeleteView):
...
class SelfInvoiceDuplicateView(LoginRequiredMixin, GroupPermissionMixin, View):
    permission_required = "invoices.add_invoice"
    """Duplicate a self-invoice: same content, new date + number."""

    @transaction.atomic
    def post(self, request, pk):
        source = get_object_or_404(SelfInvoice, pk=pk)
        new_invoice = SelfInvoice(
            type="self_invoice",
            date=date.today(),
            ...
            related_invoice_number=source.related_invoice_number,
            related_invoice_date=source.related_invoice_date,
            status="draft",
        )
        ...
        messages.success(request, f"Autofattura duplicata: {new_invoice.number}")
        return HttpResponseRedirect(reverse("self-invoices-edit", args=[new_invoice.pk]))
